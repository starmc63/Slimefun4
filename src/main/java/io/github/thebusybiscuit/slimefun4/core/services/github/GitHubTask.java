...
class GitHubTask implements Runnable {
    ...
    private void grabTextures() {
        ...
        if (requests >= MAX_REQUESTS_PER_MINUTE && Slimefun.instance() != null && Slimefun.instance().isEnabled()) {
            ...
            Bukkit.getAsyncScheduler().runDelayed(Slimefun.instance(),task -> grabTextures(),2*60,TimeUnit.SECONDS);
        }
        ...
        if (!contributor.hasTexture()) {
            try {
                ...
                if (msg != null && msg.contains("429")) {
                    Bukkit.getAsyncScheduler().runDelayed(Slimefun.instance(),task->grabTextures(),5*60,TimeUnit.SECONDS);
                }
